...
func _input(event: InputEvent) -> void:
	...
	#Open Inventory
	if event.is_action("ui_select") && event.is_pressed() && !Values.selectedItem && !Values.selectedSign && GeneralFunc.checkUI():
		print("urmom")
		Values.invOpen = true
		self.visible = true
		resetInv()
	...
	#Use Item
	elif event.is_action_pressed("ui_select") && Values.invOpen:
		print(69420)
